...
def serialize(obj, ignore=None):
    ...
    for x in obj:
        ...
        if isinstance(obj[x], datetime.datetime):
            # we fixe python bad habit to give naive timezone
            # we only works on UTC. Its quick fixe. Better use strftime and blabla
            obj[x] = str(obj[x]) + 'Z'
...
def serialize_foreign_recursive(obj, level=1, flattened=False, flaten_key=None, ignored_keys=None):
    ...
    for field in [x for x in dir(obj) if not x.startswith('_') and x != 'metadata']:
        ...
        if str(type(value)) == "<class 'sqlalchemy.orm.dynamic.AppenderQuery'>":
            ...
            if flattened:
                fields.update(serialize_foreign_recursive(value, level - 1, flattened=flattened,
                                                          flaten_key=field, ignored_keys=ignored_keys))
            else:
